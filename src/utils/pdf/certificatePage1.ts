...
export const generateCertificatePage1 = async (
  ...
  pdf.text('Certificate of Completion', pageWidth / 2, margin + 20, {
    align: 'center',
  });
  ...
  pdf.text('Learnathon 3.0 by Geeky Solutions', pageWidth / 2, margin + 30, {
    align: 'center',
  });
  ...
  pdf.setFontSize(14);
  pdf.text('This is to certify that', pageWidth / 2, margin + 60, {
    align: 'center',
  });
  ...
  const makerNames = makers.map(
    (maker) => maker.profile?.username || 'Unknown Maker'
  );
  pdf.text(makerNames.join(', '), pageWidth / 2, currentY, {
    align: 'center',
  });
  currentY += 20;
  ...
  pdf.setFontSize(14);
  pdf.setFont('helvetica', 'normal');
  pdf.text('successfully completed the project', pageWidth / 2, currentY, {
    align: 'center',
  });
  currentY += 10;
  ...
  if (overallScore !== null) {
    ...
    pdf.setTextColor(70, 60, 120);
    pdf.setFontSize(24);
    pdf.text(`${overallScore.toFixed(1)}/10`, pageWidth / 2, currentY, {
      align: 'center',
    });
    currentY += 20;
  }
  ...
  pdf.text(`Issued on ${formattedDate}`, pageWidth / 2, currentY, {
    align: 'center',
  });
  ...
  pdf.text('organized by Geeky Solutions.', pageWidth / 2, pageHeight - margin - 15, {
    align: 'center',
  });
};
